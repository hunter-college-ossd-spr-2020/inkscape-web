...
class ElectionTests(ExtraTestCase):
    ...
    def test_01_election_workflow(self):
        ...
        election._candidates.create(user_id=2, invitor_id=1)
        election._candidates.create(user_id=3, invitor_id=2)

        cs = election._candidates.all()
        response = self.assertGet('elections:accept-yes', team='e_team', slug='test-election-2011', hash=cs[0].slug)
        self.assertContains(response, 'Invitation Accepted')

        # Check Invitations lists
        self.assertContains(response, 'Testing Staff')

        # XXX Votes can NOT be submitted

        # Change election to invitations sent
        election.invitation_close()

        response = self.assertGet('elections:accept-yes', team='e_team', slug='test-election-2011', hash=cs[1].slug)
        self.assertContains(response, 'Invitation Accepted')

        # Reject invitation
        response = self.assertGet('elections:accept-no', team='e_team', slug='test-election-2011', hash=cs[2].slug)
        self.assertContains(response, 'Invitation NOT Accepted')

        # XXX Votes can NOT be submitted
        # XXX New invitation can be added, but doesn't matter
        ...
        # Log contains results
        self.assertEqual(election.log, '{...}')
