/*
 *
 * Copyright (c) 2003-2016 - Frederico Knabben (CKSource), GPLv2 or Later
 *                    2017 - Ben Stähli (django-ckeditor-link), GPLv2 or Later
 *                    2018 - Martin Owens (inkscape-web), AGPLv3 or Later
 *
 */
...
( function($) {
	CKEDITOR.plugins.add( 'cmslink', {
		...
		icons: 'CMSLink', // %REMOVE_LINE_CORE%
		...
		init: function( editor ) {
			...
			editor.on( 'doubleclick', function( evt ) {
				...
				if ( !element.isReadOnly() ) {
					if ( element.is( 'a' ) ) {
						evt.data.dialog = ( element.getAttribute( 'name' ) && ( !element.getAttribute( 'href' ) || !element.getChildCount() ) ) ? 'anchor' : 'cmslink';
			...
			editor.on( 'doubleclick', function( evt ) {
				...
				if ( evt.data.dialog in { cmslink: 1, anchor: 1 } && evt.data.link )
			...
			if ( editor.contextMenu ) {
				editor.contextMenu.addListener( function( element ) {
					...
					if ( anchor.getAttribute( 'href' ) && anchor.getChildCount() )
						menu = { cmslink: CKEDITOR.TRISTATE_OFF };
	...
	CKEDITOR.plugins.cmslink = {
		...
		/**
		 * parses link attributes into data to be used by cmslink dialog
		 */
		parseLinkAttributes: function(editor, element ) {
			...
			if (!$(element).attr("data-ckeditor-link")) {
				if ($(element).attr("href")) {
					var key = editor.config.cmslinkFallbackField;
